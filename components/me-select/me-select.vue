<template>
	<view class="me-select-item-wrap-box">
		...
		<view class="bot-fixed-wrap" v-if="editModel && data__list.length > 0 && options.showBut">
			<view v-if="selectArr.length < data__list.length" class="but position-relative" hover-class="hover-class" @tap="selectAll">全选</view>
			<view v-else class="but position-relative" hover-class="hover-class" @tap="unSelectAll">取消全选</view>
			<view @tap="finish" class="but position-relative" hover-class="hover-class" :class="{'danger': selectArr.length > 0, 'un':selectArr.length<=0 }">{{selectArr.length > 0 ? `删除(${selectArr.length})` : '删除'}}</view>
		</view>
	</view>
</template>
