<template>
	<view class="">
		<text v-if="title !== ''" class="input-top-title">交易号</text>
		<view class="me-input-wrap">
			<input :maxlength="maxlength" :minlength="minlength" v-if="type == 'password'" class="input" :name="name" :value="inputValue"
			 :type="pwdType" @focus="focus" @blur="blur" @input="handleChange" :placeholder="placeholder">
			<input :maxlength="maxlength" :minlength="minlength" v-else class="input" :name="name" :value="inputValue" :type="type"
			 @focus="focus" @blur="blur" @input="handleChange" :placeholder="placeholder">
			<text class="iconfont icon-guanbi1 icon" v-show="showDeletIcon" @tap="delUser" :class="{'right' : type == 'password'}"></text>
			<!-- <image v-show="showDeletIcon" @tap="delUser" class="icon" :class="{'right' : type == 'password'}" :src="imgInfo.icon_del" /> -->
			<text class="iconfont icon-ai-eye icon left" v-if="type == 'password'" @tap="switchPwd"></text>
			<!-- <image v-if="type == 'password'" @tap="switchPwd" class="icon left" :src="imgInfo.icon_pwd_switch" /> -->
		</view>
		<view class="line" :class="{'active': active}" />
	</view>
</template>

<script>
	export default {
		data() {
			return {
				showDeletIcon: false,
				pwdType: 'password',
				active: false,
				inputValue: '',
				imgInfo: {
					icon_del: 'data:image/png;base64,',
					icon_pwd_switch: 'data:image/png;base64,'
				}
			};
		},
		props: {
			value: {
				type: [String, Number],
				default: ''
			},
			maxlength: {
				type: Number,
				default: 255
			},
			minlength: {
				type: Number,
				default: 255
			},
			title: {
				type: String,
				default: ''
			},
			name: {
				type: String,
				default: ''
			},
			type: {
				type: String,
				default: 'text'
			},
			placeholder: {
				type: String,
				default: ''
			}
		},
		watch: {
			value: function(n) {
				this.inputValue = String(n)
				if (this.inputValue.length > 0) {
					this.showDeletIcon = true
				} else {
					this.showDeletIcon = false
				}
			}
		},
		methods: {
			focus() {
				this.active = true
				if (this.inputValue.length > 0) {
					this.showDeletIcon = true
				} else {
					this.showDeletIcon = false
				}
			},
			blur() {
				this.active = false
			},

			delUser() {
				this.showDeletIcon = false
				this.inputValue = ''
				this.$emit('input', '')
			},
			handleChange(e) {
				this.inputValue = e.detail.value
				if (this.inputValue.length > 0) {
					this.showDeletIcon = true
				} else {
					this.showDeletIcon = false
				}
				this.$emit('input', e.detail.value)

			},
			switchPwd() {
				this.pwdType = this.pwdType === 'text' ? 'password' : 'text'
			},
		}
	}
</script>

<style scoped lang="scss">
	.me-input-wrap {
		position: relative;
		width: 100%;
		max-height: 45px;



		.icon {
			position: absolute;
			right: 20upx;
			top: 50%;
			transform: translateY(-50%);
			color: #999999;
			font-size: 36upx;

			&.left {
				position: absolute;
				right: 20upx;
				top: 50%;
				transform: translateY(-50%);
			}

			&.right {
				position: absolute;
				right: 100upx;
				top: 50%;
				transform: translateY(-50%);
			}
		}

		.icon-ai-eye {
			font-size: 43upx;
		}

		.input {
			outline: none;
			height: 100upx;
			width: 100%;

			&:focus {
				outline: none;
			}
		}
	}

	.input-top-title {
		font-size: 16upx;
		color: $uni-text-color;
	}

	.line {
		width: 100%;
		height: 1px;
		background-color: $form-border-color;

		&.active {
			background-color: $uni-color-primary;
		}
	}
</style>
