	...
	<view class="serach">
		<view class="content" :style="{'border-radius':radius+'px'}">
			...
			<view class="content-box" :class="{'center':mode === 2}" @click="getFocus">
				...
				<input maxlength="18" focus :placeholder="placeholder" @input="inputChange" confirm-type="search" @confirm="triggerConfirm"
				 class="input" :focus="true" v-model="inputVal" @focus="focus" @blur="blur" />
				<!-- <view v-if="!active && mode === 2" class="input sub" @click="getFocus">请输入搜索内容</view> -->
				<!-- HM修改 @click换成@click.stop阻止冒泡 -->
				<text v-if="isDelShow" class="iconfont icon-guanbi" @click.stop="clear"></text>
			</view>
			<view v-show="inputVal.trim() !== '' && (active&&show&&button === 'inside')||(isDelShow && button === 'inside')"
			 class="serachBtn" @click="search">
				搜索
			</view>
			<view v-show="inputVal.trim() === ''" class="serachBtn" @click="backPage">
				取消
			</view>
		</view>
	...
	.serach {
		...
		.content {
			...
			.serachBtn {
				...
				padding: 0 30upx;
				//HM修改 按钮背景色
				background: linear-gradient(to right, $uni-color-primary, $uni-color-primary);
