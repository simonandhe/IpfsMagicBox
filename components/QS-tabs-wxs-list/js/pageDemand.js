	...
	getDataFn({ ...sendData
		...
		if (sendDataPageNum === 1) {
			...
			if (bl) {
				...
				observe(_this)
				_app.log('pageJS: 赋值了: setName:' + setName + ' ,newDatafields:' + newDatafields + ' ,data:' + JSON.stringify(
					newList))
			} else {
				// 不存在设置为空数组
				_this[setName] = [];
...
function observe(_this) {
	_this.$nextTick(() => {
		setTimeout(() => {
			_this.list.forEach((item, index) => {
				uni.createIntersectionObserver(_this).relativeToViewport({
					bottom: 20
				}).observe(`.image-node-${_this.tab.type}-${index}`, (res) => {
					if (res.intersectionRatio > 0) {
						if (!item.showImg) {
							_this.list[index].showImg = true
						}
					}
				})
