<template>
	<view class="container" :style="{
		...
		<!-- 	<text class="left-cir"></text>
			<text class="right-cir"></text> -->
			<scroll-view id="tabs-scroll" scroll-x :scroll-left="scrollLeft" class="tabs-scroll" scroll-with-animation>
				<view class="tabs-scroll-box" :style="{
					...
					<view class="tabs-scroll-item position-relative" hover-class="hover-class" :style="{
						'min-width': minWidth,
						'padding': '0 ' + space
					}"
					 v-for="(item, index) in tabs" :id="preId + index" :key="index" @tap="tabTap(index)">
		...
		<view class="swiper-container" :style="{
					...
					<swiper id="swiper" class="swiper" :style="{
					 ...
					 @animationfinish="QSTABSWXS.animationfinish">
						<swiper-item v-for="(item, index) in tabs" :key="index" class="swiper-item" :style="{
							...
							<block v-if="item.id === 'tab_0'">
								<template-type-all @moreClick="moreClick" @loading="loading" @loadend="loadend" ref="QSTabsWxsRef"
								 :readyRefresh="getCurrent===index?Boolean(readyRefresh):false" :show="String(readyRefresh) === 'true'?
														(getCurrent===index):
														(((index-getCurrent)<=1 && (index-getCurrent)>=-1)?
														true:false)"
								 :type="type" :current="getCurrent" :tab="item" :index="index" @refreshEnd="setRefreshStatus(4)"></template-type-all>
							</block>
							<block v-else>
								<templateDef @moreClick="moreClick" ref="QSTabsWxsRef" :readyRefresh="getCurrent===index?Boolean(readyRefresh):false"
								 :show="String(readyRefresh) === 'true'?
								(getCurrent===index):
								(((index-getCurrent)<=1 && (index-getCurrent)>=-1)?
								true:false)"
								 :type="type" :current="getCurrent" :tab="item" :index="index" @refreshEnd="setRefreshStatus(4)"></templateDef>
							</block>
						</swiper-item>
					</swiper>
				</view>
			...
			<block v-else>
				<swiper id="swiper" class="swiper" :style="{
				 ...
				 @transition="QSTABSWXS.transition" :change:tabsInfo="QSTABSWXS.tabsInfoChange" :tabsInfo="tabsInfo" :data-tabsinfo="tabsInfo"
				 :data-current="getCurrent" :data-windowwidth="windowWidth" :data-linewidth="lineWidth" :data-scrollleft="scrollLeft"
				 :data-tabsinfochangebl="tabsInfoChangeBl" @change="QSTABSWXS.swiperChange" @animationfinish="QSTABSWXS.animationfinish">
					<swiper-item v-for="(item, index) in tabs" :key="index" class="swiper-item" :style="{
						'background-color': item.swiperItemBackgroundColor || 'rgba(255,255,255,0)'
					}">
						<!-- 需自行增加列表模板并根据传入的type判断, 展示不同的列表模板 -->
						...
						<block v-else>
							<templateDef ref="QSTabsWxsRef" :readyRefresh="getCurrent===index?readyRefresh:false" :show="String(readyRefresh) === 'true'?
							(getCurrent===index):
							(((index-getCurrent)<=1 && (index-getCurrent)>=-1)?
							true:false)"
							 :type="type" :current="getCurrent" :tab="item" :index="index" @refreshEnd="setRefreshStatus(4)"></templateDef>
...
<script>
	...
	import templateTypeAll from './components/QS-tabs-wxs-template-typeAll.vue';
